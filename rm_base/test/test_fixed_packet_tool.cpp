...
TEST(FixedPacketTool, send_and_recv)
{
  ...
  auto packet_tool1 = std::make_shared<rm_base::FixedPacketTool<32>>(transporter1);
  auto packet_tool2 = std::make_shared<rm_base::FixedPacketTool<32>>(transporter2);
  rm_base::FixedPacket<32> packet1, packet2;
  // send
  int a = 10;
